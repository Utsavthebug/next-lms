...
import { getCategoriesDetail } from "@/queries/categories";
import {getReport} from '@/queries/report';

const EnrolledCourseCard = async({ enrollment }) => {
	const courseCategory = await getCategoriesDetail(enrollment.course.category);
	...
	const quizzesTaken = quizzes?.filter(quiz => quiz?.isAttempted);

	//find how many quizzes answered correctly
	const totalCorrect = quizzesTaken?.filter(quiz => {
		const item = quiz.options 
		return item.filter((o)=> o.isCorrect && o.isSelected)
					...
					<div className="flex flex-col pt-2">
						...
						<div className="border-b pb-2 mb-2">
							...
							<div className="flex items-center justify-between mt-2">
								...
								<div className="text-md md:text-sm font-medium text-slate-700">
									Quiz taken <Badge variant="success">{totalAttemptedQuizzes}</Badge>
								</div>
							</div>
							<div className="flex items-center justify-between mt-2">
								<span className="text-md md:text-sm font-medium text-slate-700">
